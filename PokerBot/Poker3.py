...
def print_cards(): # Prints the complete table with the program's cards being shown
...
def adjust(lst):
    ...
    for n in range(1, num_bots + 1):
        if program_in[n]:
            infile = open("pokergrid2","rb")
            grid = pickle.load(infile)
            prod = 1 / (1 + rate)
            if program_cards[n] in lst:
                prod = (1 + rate) ** ((num_bots + 1) ** (1 / len(lst)))
            sets = []
            sets.append(program_cards[n])
            sets.append(program_cards[n] + table_cards[:3])
            sets.append(program_cards[n] + table_cards[:4])
            sets.append(program_cards[n] + table_cards[:5])
            for i in sets:
                grid[s2(i)][int(100 * stdv(i))][len(i) - 1] *= prod ** (len(i) - 1)
            infile.close
            ...
            for i in sets:
                ...
                for j in range(4):
                    for card in i:
                       if card[1] == card_signs[j]:
                           count[j] += 1
                count = sorted(count)
                grid[count[0]][count[1]][count[2]][count[3]] *= prod ** (sum(count) - 1)
            infile.close
...
def turn(n):
    ...
    if n == 0:
        ...
        print("The pot currently has $", "", int(pot))
        probs = 1.0
        if len(program_cards[n] + table_cards) >= 5:
            best_hand = find_hand(program_cards[n] + table_cards)
            score = int(best_hand[0][0])
            #if score > 2:
                #probs *= 1.05 ** (score ** 1.5 * 2)
        ...
        infile = open("pokergrid2","rb")
        grid = pickle.load(infile)
        probs *= grid[s2(program_cards[n] + table_cards)][int(100 * stdv(program_cards[n] + table_cards))][len(program_cards[n] + table_cards) - 1]
        infile.close
        infile = open("pokersuits","rb")
        grid = pickle.load(infile)
        count = sorted(count)
        ...
        if probs < 0.5:
            ...
            chance = program_bet[n] / (max(program_bet) + program_funds[n])
...
while len(lost) < num_bots:
    ...
    for i in range(2):
        for j in range(num_bots + 1):
            if program_in[(j + a + 1) % (num_bots + 1)]:
                program_cards[(j + a + 1) % (num_bots + 1)] += [cards[i*(num_bots + 1) + j]]
            else:
                program_cards[(j + a + 1) % (num_bots + 1)] = [["  "," "],["  "," "]]
    choice(a + 3)
    for i in range (4,7):
