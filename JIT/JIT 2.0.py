## pieces wrong colors, flipped along line from top left to bottom right
## get rid of main shell completely to stop 'not responding'
## spacing is pretty good but trial and error it
...
while True:
    ...
    def print_pos2(pos):
        ...
        for i in range(len(deadblack)):
            x = deadblack[i]
            if x.lower() == 'p':
                brd.blit(bpawn, (i*55+10,0))
            if x.lower() == 'n':
                brd.blit(bknight, (i*55+10,0))
            if x.lower() == 'b':
                brd.blit(bbishop, (i*55+10,0))
            if x.lower() == 'r':
                brd.blit(brook, (i*55+10,0))
            if x.lower() == 'q':
                brd.blit(bqueen, (i*55+10,0))
            if x.lower() == 'k':
                brd.blit(bking, (i*55+10,0))
        
        for i in range(len(deadwhite)):
            x = deadwhite[i]
            if x.lower() == 'p':
                brd.blit(wpawn, (-i*55+990,1000))
            if x.lower() == 'n':
                brd.blit(wknight, (-i*55+990,1000))
            if x.lower() == 'b':
                brd.blit(wbishop, (-i*55+990,1000))
            if x.lower() == 'r':
                brd.blit(wrook, (-i*55+990,1000))
            if x.lower() == 'q':
                brd.blit(wqueen, (-i*55+990,1000))
            if x.lower() == 'k':
                brd.blit(wking, (-i*55+990,1000))
        pygame.display.update()
    ...
    def run(x, y, pos, turn, color, h):
        ...
        while True:
            if color == 1:
                ...
                pst = deepcopy(pstb)
                move, score = searcher.search(pos, secs=int(y+((check+incheck(pos)+iscornered(pos.rotate()))**2)/2))
                if score == -MATE_UPPER:
                    sys.stdout.shell.write('\n' + "\n\n    Black resigns")
                    break
                if move:
                ...
                while move not in pos.gen_moves():
                    ...
                    while nomove == 0:
                        ...
                        for event in e:
                            ...
                            if mousepos[0]>=40 && mousepos[0]<=960 && mousepos[1]>=40 && mousepos[1]<=960:
                                secondclick = 0
                                while secondclick == 0:
                                    e2 = pygame.event.get()
                                    for event2 in e2:
                                        if event2.type == pygame.MOUSEBUTTONUP:
                                            mousepos2 = pygame.mouse.get_pos()
                                                if mousepos2[0]>=40 && mousepos2[0]<=960 && mousepos2[1]>=40 && mousepos2[1]<=960:
                                                    secondclick = 1
                                user = str(ranks[7-round((mousepos[0]-82.5)/85)])+str(ranks[round((mousepos[1]-82.5)/85)])+str(ranks[7-round((mousepos2[0]-82.5)/85)])+str(ranks[round((mousepos2[1]-82.5)/85)])
                                nomove = 1  
                    if str(user) == 'no u':
                        sys.stdout.shell.write('\n' + '\n\n    Outstanding move!')
                        OM = True
