...
pstb = {#it's flipped along the line between central columns
    'P': (   0,   0,   0,   0,   0,   0,   0,   0,
            100,  110,  115,  115, 115, 115,  110,  100,
             60,   65,   70,   75,  75,  70,   65,   60,
...
def evl(depth, board, rec = False, net = 0):
    score = -999
    bestmoves = []
    if depth <= 0:
        return param(board)
    ...
    for i in board.legal_moves:
        n = net
        oldscore = param(board)
        board.push_san(str(i))
        d = depth
        if param(board) != -oldscore or board.is_check():
            d += 0.4
        elif net < -1:
            d -= 0.4
        next_i = -evl(d-0.5, board, rec = True, net = (n + param(board) + oldscore) * (param(board) + 5 - score))
        if next_i > score:
            score = next_i
            bestmoves = [str(i)]
        elif next_i == score:
            bestmoves.append(str(i))
        board.pop()
    if rec:
        return score
    return bestmoves
...
def score(board):
    ...
    for sq in range(64):
        if sq < 48 and sq > 15:
            centerness = 3.5 - abs(sq%8 - 3.5)
            if bd[2*sq].islower():
                center -= centerness
            elif bd[2*sq].isupper():
                center += centerness
        if bd[2*sq] == "K":
            for sq2 in range(64):
                if bd[2*sq2].islower() and pieces_left > 10:
                    king_safety += abs(sq2%8 - sq%8) + abs(int(sq/8) - int(sq2/8))    
        elif bd[2*sq] == "k":
            for sq2 in range(64):
                if bd[2*sq2].isupper() and pieces_left > 10:
                    king_safety -= abs(sq2%8 - sq%8) + abs(int(sq/8) - int(sq2/8))     
        elif bd[2*sq] == 'p':
            space -= int(sq/8 - 1)
            if bd[2*sq+14] == 'p' and sq%8 != 0:
                pawn_connection -= 1
            if bd[2*sq+18] == 'p' and sq%8 != 7:
                pawn_connection -= 1
        elif bd[2*sq] == 'P':
            space += int(7 - sq/8)
            if bd[2*sq-14] == 'P' and sq%8 != 7:
                pawn_connection += 1
            if bd[2*sq-18] == 'P' and sq%8 != 0:
                pawn_connection += 1
        elif bd[2*sq] == 'r':
            n = 0
            while bd[2*sq + 2*n] == '.' and int(sq/8) == int((sq + n)/8):
                n += 1
            if bd[2*sq + 2*n] == 'r' and n > 0:
                rook_connection -=1
            n = 0
            while bd[2*sq + 16*n] == '.' and (sq + 8*n) < 56:
                n += 1
            if bd[2*sq + 16*n] == 'r' and n > 0:
                rook_connection -=1
        if bd[2*sq] == 'R':
            n = 0
            while bd[2*sq + 2*n] == '.' and int(sq/8) == int((sq + n)/8):
            ...
            if bd[2*sq + 16*n] == 'R' and n > 0:
                rook_connection +=1
        elif bd[2*sq].lower() != 'q' and bd[2*sq] != ".":
            if bd[2*sq].islower():
                devment -= int(sq/8)
            else:
                devment += 7 - int(sq/8)
    return 0.05 * king_safety + 0.1 * center + 0.1 * space + 0.1 * pawn_connection + rook_connection + 0.5 * sigma(pawn_center_w) - 0.5 * sigma(pawn_center_b) + 0.5 * devment
        

def find(moves, board):
    mult = -1
    if board.turn:
        mult = 1
    best = -999
...
##        if pl == 1:
##            running = play(board)
##        moves = evl(depth, board)
##        best = find(moves, board)
##        print(param(board))
##        board.push_san(best)
##        print("\n\n\n\n\n")
##        print(board)
    ...
    while gameplay == True:
        ...
        if color == 1:
            ...
            if OM == True:
                ...
                break
            pos = pos.move(move)
            if pos.score == -MATE_UPPER and gg == 1:
                TO = True
                r = 0
                for i in pos.gen_moves():
                    if incheck(pos.move(i).rotate()) != 1:
            ...
            pst = deepcopy(pstb)
            move, score = searcher.search(pos, secs=int(x+((check+incheck(pos)+iscornered(pos.rotate()))**2)/2))
            if score == -MATE_UPPER:
                sys.stdout.shell.write('\n' + "\n\n    I resign")
                break
            if move:
            ...
            pos = pos.move(move)
            board.push_san(letters[8-move[0]%10] + str(int(move[0]/10-1)) + letters[8-move[1]%10] + str(int(move[1]/10-1)))
            saved = 0
            check = 0
            sys.stdout.shell.write('\n') 
