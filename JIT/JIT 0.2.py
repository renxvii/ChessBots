        ...
        def moves():
            ...
            for move in sorted(pos.gen_moves(), key=pos.value, reverse=True):
                ...
                if len(k) == 2:
                    ...
                    if iscornered(pos)!=1 and iscornered(pos.rotate())==1:
                        yield move, -self.bound(pos.move(move), 1-gamma, depth-1, root=False) - 200
                    elif iscornered(pos.rotate())!=1 and iscornered(pos)==1:
                        yield move, -self.bound(pos.move(move), 1-gamma, depth-1, root=False) + 200
                    else:
    ...
    while True:
        if color == 1:
            if TO == True:
                break
            pst = deepcopy(pstw)
            move, score = searcher.search(pos, secs=3*check+x+3*incheck(pos)+3*iscornered(pos.rotate()))
            if score == -MATE_UPPER:
                sys.stdout.shell.write('\n' + "\n\n    White resigns")
                break
            ...
            pst = deepcopy(pstb)
            move, score = searcher.search(pos, secs=3*check+x+y*incheck(pos)+3*iscornered(pos.rotate()))
            if score == -MATE_UPPER:
                sys.stdout.shell.write('\n' + "\n\n    Black resigns")
                break
            if move:
